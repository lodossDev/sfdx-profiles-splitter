...
class SfdxCommand extends command_1.Command {
    ...
    async assignProject() {
        ...
        try {
            this.project = await core_1.Project.resolve();
        }
        catch (err) {
    ...
    async init() {
        ...
        this.ux = new core_1.UX(this.logger, !this.isJson);
        ...
        if (ctor.varargs) {
            const argVals = lodash_1.values(args);
        ...
        //
        this.configAggregator = await core_1.SfdxConfigAggregator.create();
        // Assign this.project if the command requires to be run from within a project.
    ...
    async catch(err) {
        ...
        const userDisplayError = Object.assign({}, error.toObject(), { stack: error.stack, status: error.exitCode, result: error.data, warnings: core_1.UX.warnings });
        if (this.isJson) {
    ...
    parseVarargs(args = []) {
        ...
        if (lodash_1.get(ctor, 'varargs.required') && !args.length) {
        ...
        if (args.length) {
            args.forEach((arg) => {
                ...
                const validator = lodash_1.get(ctor, 'varargs.validator');
                if (validator) {
