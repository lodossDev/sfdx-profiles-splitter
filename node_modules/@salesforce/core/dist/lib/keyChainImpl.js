...
class GenericUnixKeychainAccess extends GenericKeychainAccess {
    async isValidFileAccess(cb) {
        const secretFile = path.join(await configFile_1.ConfigFile.resolveRootFolder(true), global_1.Global.STATE_FOLDER, keychainConfig_1.KeychainConfig.getDefaultOptions().filename);
        await super.isValidFileAccess(async (err) => {
            if (!_.isNil(err)) {
                ...
                if (octalModeStr === EXPECTED_OCTAL_PERM_VALUE) {
                    ...
                    const errorConfig = new sfdxError_1.SfdxErrorConfig('@salesforce/core', 'encryption', 'GenericKeychainInvalidPermsError', null, 'GenericKeychainInvalidPermsErrorAction', [secretFile, EXPECTED_OCTAL_PERM_VALUE]);
